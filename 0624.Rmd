...
### 5.4.3 for loop
```{r}
zoutput <- vector("{type}", length={len})
for(.x in {sequence}){
  {body}
}
```
...
#針對以上的函數我們架構了myFun函數，但它並不是vectorized function，請將它升級成vectorized function, myFun2, 使得以下指令可正常運作:
```{r}
myFun2(c(-11,32,19,20))
```

# 2. askSilly_weather 3.0
# askSilly_weather2 並不是個vectorized function, 一次要查詢多個城市時（如下）它只會回傳第一個城市的氣溫：
```{r}
...
#3.2 令 μ代表上一題的平均值，請計算其樣本標準差

# 4. 現值 明年的100元若可以在今年用90元存到，那表示利率為(100-90)/90=0.1111。反過來說，明年的100元用100/(1+0.1111)可以反推等值現在的90元價值。這1/(1+0.1111)即稱之為折現率。若利率R維持每年不變，則現金流 
```{r}
