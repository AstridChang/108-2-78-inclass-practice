...
#sort(str_sub())
sort(
  str_sub(char2,1,1) #第一個字元
)
sort(
  str_sub(char2,2,2) #第二個字元
)
```
...
startingAges <- stringr::str_extract(
  level_names,
  "^[0-9]+" # regexˋ正規表示式：開頭的一串數字
)
str(level_names)
str(startingAges) #vector,character 會無法排序
```
...
as.integer(startingAges)
 #沒有打箭頭的話只是用來顯示物件名稱，加了箭頭代表要對物件取名字，如下
...
# practice 由covid19$縣市建立一個地區變數，值為北部、中部、南部、東部，其中：
