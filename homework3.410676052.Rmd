...
class(violatedArticles) #確認class
...
# or (較佳)
table_countsByArticle[max(table_countsByArticle)] == table_countsByArticle -> mostViolatedArticle
mostViolatedArticle
```
...
```{r ans14}
calculated <- table(badEmployers$lawContent)
pick0 <- as.integer(calculated) == max(calculated)
mostViolatedArticle_content <- names(calculated[pick0])
...
# or
badEmployers[names(mostViolatedArticle) == badEmployers$law,5][[1]] -> mostViolatedArticle_content
...
factor是從1開始的整數，沒有0喔。
```{r ans2}
factor(table$`國文`,levels=table$`國文`[c(16:1)],ordered=T) ->chinese # 排序加ordered
#chinese
```
...
```{r ans22}
print(students10[c("志願","姓名")])

NTPU <- stringr::str_detect(students10$`志願`,"北大")
ntpu_namelist <- students10$`姓名`[NTPU]

#or
students10$`姓名`[str_detect(students10$`志願`,"北大")] -> ntpu_namelist
...
logical儲存的本質是甚麼?
```{r ans23}
str(students10[c("國文","英文","數學","北大經濟")])
str(students10$`志願`)
pick1 <- students10$`國文` >= 12
pick2 <- students10$`英文`+students10$`數學` >=26

ntpu_econtpu_eco <- c("陳依祈", "林溺","蔡玖")
...
(students10$`志願` == "北大經濟")*
(students10$`國文` >= 12)*
(students10$`英文` + students10$`數學` >= 26) -> picknew
as.logical(picknew)
students10$`姓名`[as.logical(picknew)]
...
```{r ans24}
# wrong
congrats <- c("恭喜陳依祈同學錄取國立臺北大學經濟系","恭喜林溺同學錄取國立臺北大學經濟系","恭喜蔡玖同學錄取國立臺北大學經濟系")
class(congrats)

# correct
ntpu_econtpu_eco[ntpu_econtpu_eco %in% namelist] -> a #確認有誰上榜
paste("恭喜", a ,"同學錄取國立臺北大學經濟系" ,sep ="") ->congrats
