...
```{r ans1}
numerals_Gujarati<-c(1,2,3,4,5,6,7,8,9)
class(numerals_Gujarati)
print(numerals_Gujarati)
sum(numerals_Gujarati)

receipts300 <- numerals_Gujarati
sum(receipts300)
...
```{r ans23}
poem_song[[22]]$paragraphs <- c(
  "斗柄仙城曉，龍鱗下隰通。", 
  "三竿眺羲馭，十里過商中。",
  "水藉天容碧，塵依驛轍紅。",
  "喜聞何力語，應爲樂秋豐。"
)
poem_song[[22]] -> poem_song
...
```{r ans33}
library(lubridate)
post$comments$DateTime <- ymd_hm("2020-04-20 14:50", tz= "Asia/Taipei")
library(lubridate)
...
```{r ans34}
library(lubridate)
post$comments[[26]] <- list(
  gender = "男",
  DateTime = ymd_hm("2020-04-25 15:50" ),
  content = "心不在課堂上，在哪都做得到遠距。",
  hearts = as.numeric(0)
)
post -> post[[26]]
post$comments$gender <- "男"
post$comments$DateTime <- ymd_hm("2020-04-25 15:50", tz="Europe/Warsaw")
post$comments$content <- "心不在課堂上，在哪都做得到遠距。"
post$comments$hearts <- as.numeric(0)
post -> post[[26]]
