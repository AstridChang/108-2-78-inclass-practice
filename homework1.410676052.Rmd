...
```{r ans44}
sum_fiveReceipts <- sum(as.numeric(fiveReceipts))
...
library(lubridate)
yy <- 2020
mm <- "Mar." #在定義object value時，必需要用「成對」的雙引號"或單引號'，把每個字串括起來。
dd <- 15
event_list <- list(
  time=ymd(paste0(yy,"",mm,dd)), #要記得用lubridate
  list(
    name="小明",
    `是否完成`=T, #要用大寫T或TRUE
    `完成所花時間(秒)`=350, #特殊命名用反頓點
    tools=c(
      "anchor","axe")
)) #少一個括號
