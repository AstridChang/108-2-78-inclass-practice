...
factor(tpe_highSchools,
       levels = c("臺北市立第一女中","台北市立第一女子高級中學","中山女中","中山女高","台北市立中山女中","臺北市立中山女子高級中學","國立政大附中","國立政治大學附屬高級中學")) -> tpe_highSchools
...
```{r ans12}
"北一女" -> levels(tpe_highSchools)[[9]] 
rep("北一女",10) -> tpe_highSchools[680:689]
tpe_highSchools
...
data228 <- data.frame(data228,
                  `全宗名編號`=factor(str_extract(data228$`檔號`,"(?<!/)[:alnum:]+(?=/)")),
                  `年`=as.integer(str_extract(data228$`檔號`,"(?<=/00)[:digit:]+(?=/)")))
data228[c("全宗名編號","年")]
...
```{r ans22}
data228$`檔號` [data228$`附註` == "表冊名稱:臺灣省二二八事變正法及死亡人犯名冊" ]

data228$`個人名稱`[data228$`附註` == "表冊名稱:臺灣省二二八事變正法及死亡人犯名冊" ] -> victims
...
```{r ans23}
documentCount_eachVictim <-sort(c(table(str_subset(data228$`個人名稱`,"吳金練")),
table(str_subset(data228$`個人名稱`,"吳斐如")),
table(str_subset(data228$`個人名稱`,"李瑞峰")),
table(str_subset(data228$`個人名稱`,"李瑞漠")),
table(str_subset(data228$`個人名稱`,"阮朝日")),
table(str_subset(data228$`個人名稱`,"林介")),
table(str_subset(data228$`個人名稱`,"邱金山")),
table(str_subset(data228$`個人名稱`,"施江南")),
table(str_subset(data228$`個人名稱`,"涂光明")),
table(str_subset(data228$`個人名稱`,"張宗仁")),
table(str_subset(data228$`個人名稱`,"許錫謙")),
table(str_subset(data228$`個人名稱`,"陳忻")),
table(str_subset(data228$`個人名稱`,"陳能通")),
table(str_subset(data228$`個人名稱`,"陳澄波")),
table(str_subset(data228$`個人名稱`,"曾鳳鳴")),
table(str_subset(data228$`個人名稱`,"湯德章")),
table(str_subset(data228$`個人名稱`,"黃何純")),
table(str_subset(data228$`個人名稱`,"黃媽典")),
table(str_subset(data228$`個人名稱`,"葉秋木")),
table(str_subset(data228$`個人名稱`,"廖進井")),
table(str_subset(data228$`個人名稱`,"潘木枝")),
table(str_subset(data228$`個人名稱`,"盧鈉欽")),
table(str_subset(data228$`個人名稱`,"蘇憲章"))),
decreasing = T)
...
```{r ans32}
courseStructure$`學年` == 103 -> pick_103
str_detect(courseStructure$`應修系級`, "[:graph:]+經濟系3[:graph:]+") -> pick_eco3
courseStructure$`科目名稱`[pick_eco3 & pick_103] -> econ101junior
...
```{r ans33}
courseStructure$`學年` == 102 -> pick_102
courseStructure$`學年` == 103 -> pick_103
courseStructure$`學年` == 104 -> pick_104
courseStructure$`學年` == 105 -> pick_105
str_detect(courseStructure$`應修系級`, "[:graph:]+經濟系1[:graph:]+") -> pick1
str_detect(courseStructure$`應修系級`, "[:graph:]+經濟系2[:graph:]+") -> pick2
str_detect(courseStructure$`應修系級`, "[:graph:]+經濟系3[:graph:]+") -> pick3
str_detect(courseStructure$`應修系級`, "[:graph:]+經濟系4[:graph:]+") -> pick4
list(
  unique(courseStructure$`科目名稱`[pick_102 & pick1]),
  unique(courseStructure$`科目名稱`[pick_103 & pick2]),
  unique(courseStructure$`科目名稱`[pick_104 & pick3]),
  unique(courseStructure$`科目名稱`[pick_105 & pick4])
) -> coursesOffered
