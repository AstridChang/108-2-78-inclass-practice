...
```{r ans22}
course_update <- course[[2]]$course[["type"]] <- "選"
course_update <- course[[1]]$approve <- TRUE
str(course_update)
...
```{r ans23}
course_update2 <- course$course <- list(teacher="紀學檜老師")
str(course_update2)
# course_update2
```
...
```{r ans32}
Pikachu_stats <- Gamemaster[c("atk","def","hp")]
as.integer(Pikachu_stats)
class(Pikachu_stats)
# Pikachu_stats
```
...
str(Jigglypuff)
Jigglypuff <- Gamemaster[[2]]$speciesName <- "Jigglypuff"
str(Jigglypuff)
# Jigglypuff
```
