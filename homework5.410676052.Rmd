...
a1<-sort(as.integer(levels(studentHeights$`學年度`)))
a2<-as.character(a1)

b1<-sort(as.integer(levels(studentHeights$`年齡`)))
b2<-as.character(b1)
...
### 1.3 各年齡層平均身高
請創造一個named numeric vector, averageHeightByGroup (class numeric, length=3) 其值分別是三組年齡層各自所包含年齡下不分性別身高的平均（即第一個值是所有6-8歲不分學年度、不分性別身高（總計身高）的平均，再來是9-12歲，最後是13-15歲），而此vector的元素名稱也分別是"6-8", "9-12", "13-15" (hint: 數值向量x元素平均, 可用mean(x)求得； 這題也考驗同學組合數個「要不要」向量`pick_XXX`) (#4.4 names)

```{r ans13}
one <- mean(studentHeights$`平均身高`[studentHeights$`年齡層` == "6-8"])
two <- mean(studentHeights$`平均身高`[studentHeights$`年齡層` == "9-12"])
three <- mean(studentHeights$`平均身高`[studentHeights$`年齡層` == "13-15"])
combined <- c(one,two,three)
names(combined) <- c("6-8", "9-12", "13-15")

combined -> averageHeightByGroup
...
```{r ans14}
boy1 <- mean(studentHeights$`平均身高`[(studentHeights$`年齡層` == "6-8") & (studentHeights$`類別` == "男")])
boy2 <- mean(studentHeights$`平均身高`[(studentHeights$`年齡層` == "9-12") & (studentHeights$`類別` == "男")])
boy3 <- mean(studentHeights$`平均身高`[(studentHeights$`年齡層` == "13-15") & (studentHeights$`類別` == "男")])

girl1 <-  mean(studentHeights$`平均身高`[(studentHeights$`年齡層` == "6-8") & (studentHeights$`類別` == "女")])
girl2 <-  mean(studentHeights$`平均身高`[(studentHeights$`年齡層` == "9-12") & (studentHeights$`類別` == "女")])
girl3 <-  mean(studentHeights$`平均身高`[(studentHeights$`年齡層` == "13-15") & (studentHeights$`類別` == "女")])
...
```{r ans22}
currentLevels <- factor(differentMajors)
levels(currentLevels)
class(levels(currentLevels))
...
currentsLevels <- levels(differentMajors)
business_regex <- "休運系|金融系"
social_regex <- "經濟系|社會系|社工系"
public_regex <- "公行系|不動系"
currentsLevels <- str_replace(currentsLevels,business_regex,"商學院")
currentsLevels <- str_replace(currentsLevels,social_regex,"社科院")
currentsLevels <- str_replace(currentsLevels,public_regex,"公共事務學院")
currentsLevels
school <- factor(currentsLevels)
class(school)
levels(school)
